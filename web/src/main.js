...
function connect() {
  ...
  state.socket.on("message", async packet => {
    try {
      ...
      drawMessages();
      if (state.sound) beep();
      if (document.hidden || state.notifications) notifyUser("New message", state.preview ? text : "You have a new message");
    } catch {}
  });
...
async function encrypt(text,from,to) {
  const key=await keyFor(from,to),iv=crypto.getRandomValues(new Uint8Array(12));
...
async function decrypt(ciphertext,iv64,me) {
  const key=await keyFor(state.peer,me),iv=Uint8Array.from(atob(iv64),c=>c.charCodeAt(0)),data=Uint8Array.from(atob(ciphertext),c=>c.charCodeAt(0));
...
async function enableNotifications() {
  if (!("Notification" in window)) { alert("Notifications are not supported here."); return false; }
  const permission=await Notification.requestPermission();
  if(permission==="granted"){
    state.notifications=true; localStorage.setItem("cocoon_notifications","on");
    if ("serviceWorker" in navigator) {
      try { notificationRegistration = await navigator.serviceWorker.ready; } catch {}
    }
    notifyUser("Cocoon notifications enabled", "Notifications are active.");
    return true;
  }
  state.notifications=false; localStorage.setItem("cocoon_notifications","off");
...
async function notifyUser(title, body) {
  ...
  try {
    ...
    if (notificationRegistration?.showNotification) {
      await notificationRegistration.showNotification(title,{body,tag:"cocoon",renotify:true,icon:"/favicon.ico",silent:!state.sound,data:{url:"/"}});
    } else if (!document.hidden) {
      new Notification(title,{body,silent:!state.sound});
    }
  } catch {}
}
...
function beep() {
  ...
  const o=c.createOscillator(),g=c.createGain();o.frequency.value=660;g.gain.value=.035;o.connect(g);g.connect(c.destination);
  o.start();o.stop(c.currentTime+.10);
}
function startRingtone() {
  stopRingtone();
  const play=()=>{const c=getAudioContext();if(!c)return;const o=c.createOscillator(),g=c.createGain();o.type="sine";o.frequency.value=880;g.gain.value=.035;o.connect(g);g.connect(c.destination);o.start();o.stop(c.currentTime+.18);};
  play(); ringtoneTimer=setInterval(play,1200);
}
...
["click","keydown","touchstart"].forEach(evt=>document.addEventListener(evt,()=>{getAudioContext();resetIdleTimer();},{passive:true}));
document.addEventListener("visibilitychange",()=>{if(document.hidden && state.autoLock && localStorage.getItem(PIN_KEY)) lockApp("hidden");});
